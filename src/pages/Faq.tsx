...
const faqItems = [
  ...
  ['How does Live Slideshow work at a wedding?', 'Open the Live Slideshow on a venue touchscreen, tablet, laptop or desktop display. Guests can approach the device, search their name and find their table or seat.'],
...
export const Faq = () => {
  ...
  return <div className="ww-public min-h-screen"><SeoHead title="Wedding Waitress FAQ | Plans, Guests, QR & Products" description="Answers about the Wedding Waitress trial, couple plans, guest limits, QR seating, stationery, floor plans, Live Slideshow, photo sharing and event-day tools." canonicalPath="/faq" jsonLd={schema} /><Header /><main><PublicPageHero compact asset={publicHeroForRoute('/faq')} eyebrow="Frequently asked questions" title="Answers before you start planning" description="Clear information about the platform, plans and the way guests and suppliers use Wedding Waitress." /><section className="ww-section"><div className="ww-container max-w-4xl"><div className="divide-y border-y">{faqItems.map(([q,a]) => <details key={q} className="group py-5"><summary className="ww-focus cursor-pointer list-none rounded pr-8 text-lg font-semibold">{q}<span className="float-right group-open:rotate-45" aria-hidden="true">+</span></summary><p className="mt-4 max-w-3xl leading-7 text-[#6f625b]">{a}</p></details>)}</div><p className="mt-9 text-center text-[#6f625b]">Still need help? <Link to="/contact" className="ww-public-link font-semibold">Contact Wedding Waitress</Link>.</p></div></section></main><PublicFooter /><CookieBanner /></div>;
};
