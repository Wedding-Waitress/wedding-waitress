...
describe('shared public header logo', () => {
  ...
  it('aligns the header logo to the footer content container', () => {
    ...
    expect(header).toContain('mx-auto flex max-w-7xl items-center justify-between');
    expect(header).toContain("!user ? 'min-[1320px]:justify-end min-[1320px]:gap-2' : ''");
    expect(header).toContain("!user ? 'min-[1320px]:pr-3' : ''");
