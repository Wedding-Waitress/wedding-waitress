...
import { SeatingChartShareToken } from '@/hooks/useSeatingChartShare';
...
interface SeatingChartShareModalProps {
  ...
  shareTokens: SeatingChartShareToken[];
  onGenerateToken: (permission: 'view_only', recipientName?: string, validityDays?: number) => Promise<string | null>;
  onDeleteToken: (tokenId: string) => void;
}

export function SeatingChartShareModal({
  open,
  onOpenChange,
  shareTokens,
  onGenerateToken,
  onDeleteToken,
}: SeatingChartShareModalProps) {
  ...
  const handleGenerate = useCallback(async () => {
    ...
    setGenerating(false);
    if (token) {
      const url = buildSeatingChartUrl(token);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Share the full seating chart with your venue coordinator, wedding planner, or vendors.
          </DialogDescription>
        </DialogHeader>
          ...
          <TabsContent value="create" className="space-y-4 mt-4">
            <div className="space-y-2">
              ...
              <Input
                ...
                placeholder="e.g., Venue Coordinator, Wedding Planner"
              />
            </div>
                  ...
                  <div key={token.id} className="flex items-center justify-between p-3 bg-muted/50 rounded-lg">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <Button variant="ghost" size="icon" className="h-8 w-8" title="Open Link" asChild>
                        <a href={buildSeatingChartUrl(token.token)} target="_blank" rel="noopener noreferrer">
