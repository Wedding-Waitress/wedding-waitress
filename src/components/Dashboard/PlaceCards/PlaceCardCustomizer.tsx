        ...
        <Tabs defaultValue="design" className="space-y-4">
          ...
          <TabsContent value="background" className="space-y-4">
            <div className="space-y-4">
              ...
              {/* Standard background image upload for decorative and full modes */}
              {(currentSettings.background_image_type === 'decorative' || currentSettings.background_image_type === 'full') && <div>
              ...
              {/* Front of card image upload */}
              {currentSettings.background_image_type === 'full_front' && <div>
                  <Label className="mb-2 block">Upload Front of Card Image</Label>
                  <div className="space-y-2">
                    <input
                      type="file"
                      accept="image/*"
                      onChange={async (event) => {
                        ...
                        try {
                          ...
                          const fileExt = file.name.split('.').pop();
                          const fileName = `place-card-front-${Date.now()}.${fileExt}`;
                          const filePath = `${user.id}/place-cards/${fileName}`;
                          const { error: uploadError } = await supabase.storage.from('qr-assets').upload(filePath, file);
                          if (uploadError) throw uploadError;
                          const { data: { publicUrl } } = supabase.storage.from('qr-assets').getPublicUrl(filePath);
                          await handleSettingChange('front_image_url', publicUrl);
                          toast({ title: "Success", description: "Front image uploaded successfully" });
                        } catch (error) {
                          console.error('Error uploading image:', error);
                    ...
                    <div className="flex gap-2">
                      ...
                      {(currentSettings as any).front_image_url && (
                        <Button
                          variant="destructive"
                          size="sm"
                          onClick={async () => {
                            await handleSettingChange('front_image_url', null);
                            toast({ title: "Image Removed", description: "Front image has been removed" });
                          }}
                          className="flex-1 rounded-full flex items-center justify-center gap-2"
                        >
                          <Trash2 className="h-4 w-4" />
                    ...
                    {(currentSettings as any).front_image_url && (
                      <div className="mt-2 space-y-2">
                        <img src={(currentSettings as any).front_image_url} alt="Front preview" className="w-full h-auto object-contain rounded border max-h-32" />
                        <div className="flex items-center gap-2 text-sm text-muted-foreground">
                          <Image className="h-4 w-4" />
                          <span>This image will appear on the front of the card (guest name side)</span>
                        </div>
                      </div>
                    )}
                  </div>
              ...
              {/* Back of card image upload */}
              {currentSettings.background_image_type === 'full_back' && <div>
                  <Label className="mb-2 block">Upload Back of Card Image</Label>
                  <div className="space-y-2">
                    <input
                      type="file"
                      accept="image/*"
                      onChange={async (event) => {
                        ...
                        try {
                          ...
                          if (authError || !user) throw new Error('User not authenticated');
                          const fileExt = file.name.split('.').pop();
                          const fileName = `place-card-back-${Date.now()}.${fileExt}`;
                          const filePath = `${user.id}/place-cards/${fileName}`;
                          const { error: uploadError } = await supabase.storage.from('qr-assets').upload(filePath, file);
                          if (uploadError) throw uploadError;
                          const { data: { publicUrl } } = supabase.storage.from('qr-assets').getPublicUrl(filePath);
                          await handleSettingChange('back_image_url', publicUrl);
                          toast({ title: "Success", description: "Back image uploaded successfully" });
                        } catch (error) {
                          console.error('Error uploading image:', error);
                          toast({ title: "Error", description: "Failed to upload image", variant: "destructive" });
                        } finally {
                    ...
                    <div className="flex gap-2">
                      ...
                      {(currentSettings as any).back_image_url && (
                        <Button
                          variant="destructive"
                          size="sm"
                          onClick={async () => {
                            await handleSettingChange('back_image_url', null);
                            toast({ title: "Image Removed", description: "Back image has been removed" });
                          }}
                          className="flex-1 rounded-full flex items-center justify-center gap-2"
                        >
                          <Trash2 className="h-4 w-4" />
                    ...
                    {(currentSettings as any).back_image_url && (
                      <div className="mt-2 space-y-2">
                        <img src={(currentSettings as any).back_image_url} alt="Back preview" className="w-full h-auto object-contain rounded border max-h-32" />
                        <div className="flex items-center gap-2 text-sm text-muted-foreground">
                          <Image className="h-4 w-4" />
                          <span>This image will appear on the back of the card (rotated for tent fold)</span>
                        </div>
                      </div>
                    )}
                  </div>
