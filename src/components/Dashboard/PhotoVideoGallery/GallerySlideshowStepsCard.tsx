...
import { CheckCircle2, MonitorPlay, RefreshCw, HelpCircle } from 'lucide-react';

const STEPS = [
  { icon: CheckCircle2, title: 'Approve your media', text: 'Approve the guest photos and videos you want displayed.' },
  { icon: MonitorPlay, title: 'Open on a big screen', text: 'Open the Live Slideshow on a television, monitor or projector.' },
  { icon: RefreshCw, title: 'It updates itself', text: 'Newly approved uploads automatically join the slideshow.' },
];
  ...
  <Card className="h-full p-5 sm:p-6 space-y-5 overflow-hidden">
    <div className="min-w-0">
      <h2 className="text-xl font-bold flex items-center gap-2" style={{ color: '#000000' }}>
        <HelpCircle className="h-5 w-5 text-[#967A59] shrink-0" /> How the Live Slideshow Works
      </h2>
      ...
      {STEPS.map((s, i) => (
        <li key={s.title} className="flex items-start gap-3 min-w-0">
          <div className="w-9 h-9 rounded-full bg-[#967A59]/10 flex items-center justify-center shrink-0">
            <s.icon className="h-[18px] w-[18px] text-[#967A59]" />
          </div>
          <div className="min-w-0">
