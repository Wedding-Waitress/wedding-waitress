...
export const PhotoBoothTemplatePreview: React.FC<Props> = ({ kind, opts, portrait = true }) => {
  ...
  return (
    <div className="rounded-md border border-border bg-[#F7F5F1] overflow-hidden flex items-center justify-center min-h-[280px] py-3 px-1.5">
      {busy && !url ? (
        ...
        <img
          ...
          className={`max-h-[700px] w-auto max-w-full object-contain shadow-sm ${busy ? 'opacity-60' : ''}`}
        />
      ) : null}
    </div>
