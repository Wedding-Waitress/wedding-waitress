...
// Verification runs server-side (rate-limited edge function). Stores a
// per-token "verified" flag in sessionStorage so guests aren't re-prompted
// on every navigation/refresh during the same session.
...
export const GalleryPasswordGate: React.FC<Props> = ({ token, title, onVerified, variant = 'light', theme }) => {
  ...
  // Display-only countdown. Enforcement is entirely server-side: refreshing
  // the page or clearing this state does not lift the block.
  useEffect(() => {
  ...
  const submit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!pw.trim() || blocked) return;
    if (inFlight.current) return; // guard against double-clicks
    inFlight.current = true;
    setLoading(true);
    setErr(null);
    try {
      const res = await verifyGalleryPassword(token, pw);
      if (res.rateLimited) {
        setRetryAfter(res.retryAfterSeconds);
        setErr(blockedMessage(res.retryAfterSeconds));
      } else if (res.ok) {
        try { sessionStorage.setItem(galleryPasswordKey(token), pw); } catch {}
        onVerified(pw);
      } else {
        setErr(res.error || 'Incorrect password. Please try again.');
      }
    } catch {
