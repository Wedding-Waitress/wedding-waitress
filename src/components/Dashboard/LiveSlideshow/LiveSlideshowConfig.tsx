...
interface LiveSlideshowConfigProps {
  eventId: string;
}
...
export const LiveSlideshowConfig: React.FC<LiveSlideshowConfigProps> = ({ eventId }) => {
    ...
    <Card id="guest-live-view-configuration" className={`${styles.primaryPanel} border border-primary shadow-[0_4px_20px_-4px_rgba(0,0,0,0.15)] w-full scroll-mt-24`}>
      ...
      <CardContent className="space-y-3">
        <p className={`${styles.configurationDescription} text-sm text-muted-foreground`}>
          Configure which modules your guests can access from the Live Slideshow.
        </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3 items-stretch auto-rows-fr">
          {(() => {
            ...
            const togglesTile = (
              <div key="toggles" className={`${styles.tile} h-full flex flex-col space-y-3 p-4 rounded-lg border-2 border-primary bg-muted/20 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.15)]`}>
                <h4 className="text-sm font-semibold flex items-center gap-[7px]">
                  ...
                  Slideshow Display Toggles
                </h4>
                <p className="text-xs text-muted-foreground">
                  Control which guest details appear on the Live Slideshow result card.
                </p>
            ...
            // Row 2: Menu | Reception Floor Plan | Slideshow Display Toggles
            const rsvp = tiles[0];
