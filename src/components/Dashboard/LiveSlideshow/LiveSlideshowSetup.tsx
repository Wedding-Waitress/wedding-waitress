...
import { buildLiveSlideshowUrl } from '@/lib/urlUtils';
import { LiveSlideshowConfig } from './LiveSlideshowConfig';
import { LiveSlideshowAutoProtection } from './LiveSlideshowAutoProtection';
import styles from './LiveSlideshowSetup.module.css';

interface LiveSlideshowSetupProps {
...
export const LiveSlideshowSetup: React.FC<LiveSlideshowSetupProps> = ({
  selectedEventId, 
  onEventSelect,
  events,
  eventsLoading,
  ...
  const [isOpeningSlideshow, setIsOpeningSlideshow] = useState(false);

  const selectedEvent = selectedEventId ? events.find(e => e.id === selectedEventId) : null;
  const liveSlideshowUrl = selectedEvent?.slug ? buildLiveSlideshowUrl(selectedEvent.slug) : '';

  const handleCopyUrl = async () => {
    if (!liveSlideshowUrl) return;
    
    try {
      await navigator.clipboard.writeText(liveSlideshowUrl);
      setCopied(true);
      setTimeout(() => setCopied(false), 2000);
      toast({
        title: "URL Copied",
        description: "Live Slideshow URL has been copied to clipboard",
      });
    } catch (error) {
      toast({
        title: "Copy Failed",
  ...
  const handleOpenLiveSlideshow = () => {
    if (!liveSlideshowUrl) return;
    setIsOpeningSlideshow(true);
    const opened = window.open(liveSlideshowUrl, '_blank');
    if (!opened) {
      toast({
        title: 'Popup blocked',
        description: 'Allow pop-ups for this site to open the Live Slideshow window.',
        variant: 'destructive',
      });
    }
    setTimeout(() => setIsOpeningSlideshow(false), 800);
  };

  const handleFullscreen = () => {
    if (!liveSlideshowUrl) return;
    const newWindow = window.open(liveSlideshowUrl, '_blank', 'fullscreen=yes,scrollbars=no,toolbar=no,menubar=no,status=no');
    if (newWindow) {
  ...
  React.useEffect(() => {
    ...
    import('qrcode').then(({ default: QRCode }) => QRCode.toDataURL(liveSlideshowUrl, {
      width: 200,
      margin: 2,
  ...
  return (
    <div className={`${styles.page} space-y-6 md:max-lg:space-y-8 md:max-lg:px-2`}>
      {/* Consolidated header, event selection, and Live Slideshow controls. */}
      <Card className={`${styles.primaryPanel} border border-primary shadow-[0_4px_20px_-4px_rgba(0,0,0,0.15)]`}>
        <CardHeader>
          {/* Live Slideshow heading and description on one row. */}
          <div className="flex flex-col sm:flex-row sm:items-center gap-3">
            ...
            <div className="flex flex-col lg:flex-row lg:items-baseline gap-1 lg:gap-3">
              <CardTitle className={`${styles.mainHeading} ww-live-slideshow-main-heading text-2xl font-bold text-foreground`}>Live Slideshow</CardTitle>
              <CardDescription className={`${styles.mainDescription} shrink-0`}>
                Configure a self-service guest lookup slideshow for weddings, corporate events, workshops and seminars
              </CardDescription>
            </div>
          </div>
        ...
        <CardContent className="pt-6 space-y-6">
          ...
          {/* Three equal boxes: Live Slideshow controls, instructions, and auto-protection. */}
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 items-stretch">
            {/* Live Slideshow URL and controls. */}
            <div className={`${styles.innerPanel} h-full rounded-lg border border-[#472c1d] p-4 space-y-3`}>
              <div>
                <h3 className="flex items-center gap-2 text-[20px] font-bold text-[#472c1d]">
                  ...
                  Live Slideshow URL &amp; Controls
                </h3>
                <CardDescription className="mt-1">
                  Use this URL to present the Live Slideshow on your chosen device
                </CardDescription>
              </div>

              {!selectedEvent ? (
                <p className="text-sm text-muted-foreground">
                  Choose an event above to generate your Live Slideshow link and QR code.
                </p>
              ) : (
                <div className="flex flex-col items-center gap-4">
                  ...
                  <div className="flex flex-col items-center text-center w-full">
                    <h4 className="text-sm font-semibold mb-2 inline-flex items-center justify-center gap-2">
                      ...
                      QR Code for Live Slideshow
                    </h4>
                    {qrCodeDataUrl ? (
                      <img
                        src={qrCodeDataUrl}
                        alt="Live Slideshow QR code"
                    ...
                    <p className="text-xs text-muted-foreground mt-2 max-w-[220px] leading-snug">
                      Scan to open the Live Slideshow on a tablet or mobile device
                    </p>
                  </div>

                  {/* URL + Actions — stacked full-width */}
                  <div className="flex flex-col gap-2 w-full">
                    <code className={`${styles.urlField} text-xs break-all block p-2 bg-muted rounded-md border`}>{liveSlideshowUrl}</code>
                    <button
                      ...
                      aria-label="Copy Live Slideshow URL"
                    >
                      {copied ? (
                    ...
                    <button
                      onClick={handleOpenLiveSlideshow}
                      disabled={isOpeningSlideshow}
                      aria-label="Open Live Slideshow in a new tab"
            ...
            <div className={`${styles.innerPanel} h-full rounded-lg border border-[#472c1d] p-5 space-y-4`}>
              ...
              <div className="space-y-3">
                <div className="flex items-start gap-3">
                  ...
                  <p className="text-sm">
                    <strong>Choose your device:</strong> Use a tablet, laptop or desktop computer for the Live Slideshow
                  </p>
                </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <p className="text-sm">
                    <strong>Open the Live Slideshow URL:</strong> Navigate to the Live Slideshow URL on your chosen device
                  </p>
                </div>
                ...
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    ...
                    <ul className="text-sm text-foreground/80 space-y-1">
                      ...
                      <li>• Consider adding a sign explaining how guests can use the Live Slideshow</li>
                    </ul>
                  </div>
                </div>
              </div>
            ...
            <div className={`${styles.innerPanel} h-full rounded-lg border border-[#472c1d] p-4 md:col-span-2 xl:col-span-1`}>
              {selectedEvent ? (
                <LiveSlideshowAutoProtection eventId={selectedEvent.id} />
              ) : (
                <p className="text-sm text-muted-foreground">
      ...
      {selectedEvent && <LiveSlideshowConfig eventId={selectedEvent.id} />}
    </div>
  );
};
