...
async function processExport(
  ...
  try {
    ...
    const textPostsFolder = zip.folder('text-posts');
    ...
    for (const item of mediaItems) {
      ...
      if (item.post_type === 'photo' && item.file_url) {
        try {
          ...
          if (!downloadError && fileData) {
            ...
            itemData.exported_filename = `photos/${filename}`;
      ...
      if (item.post_type === 'video' && item.file_url && !item.cloudflare_stream_uid) {
        try {
          ...
          if (!downloadError && fileData) {
            ...
            itemData.exported_filename = `videos/${filename}`;
        ...
        if (item.stream_preview_image) {
          try {
            ...
            if (thumbnailResponse.ok) {
              ...
              thumbnailsFolder?.file(`stream_${item.cloudflare_stream_uid}.jpg`, thumbnailBlob);
            }
          } catch (err) {
      ...
      if (item.post_type === 'text' && item.text_content) {
        ...
        textPostsFolder?.file(filename, item.text_content);
        itemData.exported_filename = `text-posts/${filename}`;
      }
...
- photos/ - All photo files with original quality
- videos/ - Videos uploaded directly (not via Cloudflare Stream)
- thumbnails/ - Thumbnails for Cloudflare Stream videos
- text-posts/ - Guest book messages and text posts
...
Instead, see videos.csv for playback URLs and metadata.
