...
ALTER TABLE public.event_media_galleries
  ADD COLUMN IF NOT EXISTS photo_booth_enabled boolean NOT NULL DEFAULT false;

ALTER TABLE public.event_media_items
  ADD COLUMN IF NOT EXISTS is_photo_booth boolean NOT NULL DEFAULT false;
...
BEGIN
  UPDATE public.event_media_galleries
     SET photo_booth_enabled = _enabled, updated_at = now()
   WHERE event_id = _event_id AND user_id = auth.uid();
  IF NOT FOUND THEN RAISE EXCEPTION 'Gallery not found'; END IF;
END;
$$;
GRANT EXECUTE ON FUNCTION public.set_event_media_photo_booth(uuid, boolean) TO authenticated;
...
BEGIN
  ...
  SELECT g.id, g.is_open,
    ...
    g.video_guestbook_enabled, g.photo_booth_enabled
...
RETURNS TABLE(id uuid, kind event_media_kind, mime_type text, byte_size bigint, duration_sec integer, storage_path text, uploader_name text, caption text, guestbook_message text, uploaded_at timestamp with time zone, moderation_status text, album text, is_guestbook boolean, is_photo_booth boolean)
LANGUAGE plpgsql STABLE SECURITY DEFINER SET search_path TO 'public'
AS $$
BEGIN
  RETURN QUERY
  SELECT i.id, i.kind, i.mime_type, i.byte_size, i.duration_sec,
         ...
         i.uploaded_at, i.moderation_status, i.album, i.is_guestbook, i.is_photo_booth
...
BEGIN
  ...
  _path := _gal.event_id::text || '/photobooth/' || _item_id::text || '.' || _ext;
  ...
  INSERT INTO public.event_media_items (
    ...
    NULLIF(trim(_uploader_name), ''), true,
...
END;
$function$;
GRANT EXECUTE ON FUNCTION public.register_event_photobooth_upload(text, text, bigint, text, text) TO anon, authenticated;
