...
ALTER TABLE running_sheet_items ADD COLUMN IF NOT EXISTS is_bold boolean DEFAULT false;
ALTER TABLE running_sheet_items ADD COLUMN IF NOT EXISTS is_italic boolean DEFAULT false;
ALTER TABLE running_sheet_items ADD COLUMN IF NOT EXISTS is_underline boolean DEFAULT false;

CREATE OR REPLACE FUNCTION public.get_running_sheet_by_token(share_token text)
 RETURNS TABLE(sheet_id uuid, event_id uuid, event_name text, event_date date, event_venue text, start_time time without time zone, finish_time time without time zone, ceremony_date date, ceremony_venue text, ceremony_start_time time without time zone, ceremony_finish_time time without time zone, permission text, items jsonb)
 LANGUAGE plpgsql
 SECURITY DEFINER
