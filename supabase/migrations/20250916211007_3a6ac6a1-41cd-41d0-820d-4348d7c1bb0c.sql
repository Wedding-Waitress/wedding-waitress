...
DROP POLICY IF EXISTS "Users can upload their own QR assets" ON storage.objects;
DROP POLICY IF EXISTS "Users can view their own QR assets" ON storage.objects;
DROP POLICY IF EXISTS "Users can update their own QR assets" ON storage.objects;
DROP POLICY IF EXISTS "Users can delete their own QR assets" ON storage.objects;
DROP POLICY IF EXISTS "QR assets are publicly accessible" ON storage.objects;

-- Create policies for QR assets storage
CREATE POLICY "Users can upload their own QR assets" 
ON storage.objects 
FOR INSERT 
WITH CHECK (bucket_id = 'qr-assets' AND auth.uid()::text = (storage.foldername(name))[1]);

CREATE POLICY "Users can view their own QR assets" 
ON storage.objects 
FOR SELECT 
USING (bucket_id = 'qr-assets' AND auth.uid()::text = (storage.foldername(name))[1]);

CREATE POLICY "Users can update their own QR assets" 
ON storage.objects 
FOR UPDATE 
USING (bucket_id = 'qr-assets' AND auth.uid()::text = (storage.foldername(name))[1]);

CREATE POLICY "Users can delete their own QR assets" 
ON storage.objects 
FOR DELETE 
USING (bucket_id = 'qr-assets' AND auth.uid()::text = (storage.foldername(name))[1]);

CREATE POLICY "QR assets are publicly accessible" 
ON storage.objects 
FOR SELECT 
USING (bucket_id = 'qr-assets');

-- Add trigger for qr_code_settings updated_at
DROP TRIGGER IF EXISTS update_qr_code_settings_updated_at ON public.qr_code_settings;
