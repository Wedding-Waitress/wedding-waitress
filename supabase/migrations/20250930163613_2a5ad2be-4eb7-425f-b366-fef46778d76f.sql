-- Drop and recreate get_public_event_with_data function to include rsvp_deadline
DROP FUNCTION IF EXISTS public.get_public_event_with_data(text);

CREATE FUNCTION public.get_public_event_with_data(event_slug text)
RETURNS TABLE(
  event_id uuid,
  event_name text,
  event_date text,
...
AS $$
  SELECT 
    ...
    e.partner1_name,
    e.partner2_name,
    e.rsvp_deadline::text as event_rsvp_deadline,
    g.id as guest_id,
    ...
    g.relation_display as guest_relation_display,
    t.id as table_id,
